                                                        ...
                                                        <td class="text-center">
                                                            <div class="dropdown btn-group">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-animated">
                                                                    <?php if ($item->status == 1) : ?>
                                                                        <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal" data-target="#modal-country" onclick="$('form').attr('action', '/admin_controller/saved_country_post'); $('#modal-modalLabel').text('<?php echo trans('edit'); ?>'); $('#modal_id').val('<?php echo html_escape($item->id); ?>'); $('#modal_name').val('<?php echo html_escape($item->name); ?>'); $('#modal_code').val('<?php echo html_escape($item->continent_code); ?>'); $('#status_1').prop('checked', true);"><?php echo trans('edit'); ?></a>
                                                                    <?php else : ?>
                                                                        <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal" data-target="#modal-country" onclick="$('form').attr('action', '/admin_controller/saved_country_post'); $('#modal-modalLabel').text('<?php echo trans('edit'); ?>'); $('#modal_id').val('<?php echo html_escape($item->id); ?>'); $('#modal_name').val('<?php echo html_escape($item->name); ?>'); $('#modal_code').val('<?php echo html_escape($item->continent_code); ?>'); $('#status_2').prop('checked', true);"><?php echo trans('edit'); ?></a>
                                                                    <?php endif; ?>
                                                                    <div class=" dropdown-divider">
                                                                    </div>
                                                                    <a class="dropdown-item" href="javascript:void(0)" onclick="delete_item('admin_controller/delete_country_post','<?php echo $item->id; ?>','<?php echo trans('confirm_delete'); ?>');"><?php echo trans('delete'); ?></a>
                                                                </div>
                                                            </div>
...
<div id="modal-country" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modal-modalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="form_safe" action="/admin_controller/saved_country_post" method="post">
